...
int getArrival(Job J) {
  if (J == NULL) {
    fprintf(stderr, "Job Error: calling getArrival() on NULL Job reference\n");
    exit(EXIT_FAILURE);
  }
...
int getDuration(Job J) {
  if (J == NULL) {
    fprintf(stderr, "Job Error: calling getDuration() on NULL Job reference\n");
    exit(EXIT_FAILURE);
  }
...
int getFinish(Job J) {
  if (J == NULL) {
    fprintf(stderr, "Job Error: calling getFinish() on NULL Job reference\n");
    exit(EXIT_FAILURE);
  }
...
int getWaitTime(Job J) {
  if (J == NULL) {
    fprintf(stderr, "Job Error: calling getWaitTime() on NULL Job reference\n");
    exit(EXIT_FAILURE);
  }
  if ((J->finish) == UNDEF) {
    fprintf(stderr,
...
void computeFinishTime(Job J, int timeNow) {
  if (J == NULL) {
    fprintf(stderr, "Job Error: computeFinishTime() on NULL Job reference\n");
    exit(EXIT_FAILURE);
  }
...
void resetFinishTime(Job J) {
  if (J == NULL) {
    fprintf(stderr, "Job Error: resetFinishTime() on NULL Job reference\n");
    exit(EXIT_FAILURE);
  }
...
void printJob(FILE *out, Job J) {
  if (J == NULL) {
    fprintf(stderr, "Job Error: printJob() on NULL Job reference\n");
    exit(EXIT_FAILURE);
  }
